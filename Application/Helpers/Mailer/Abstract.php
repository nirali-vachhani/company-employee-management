...
abstract class Helpers_Mailer_Abstract
{
    ...
    public function addRecipient($email,$name)
    {
        ...
        //in staging & dev environments
        if(ENV ==  "development" || ENV == "staging")
        {
            $this->clearRecipients();
            array_push($this->recipients, array('email'=>DEBUG_EMAIL,'name'=>'Looyalty Emails'));
            if(defined('DEBUG_EMAIL2'))
            {
                array_push($this->recipients, array('email'=>DEBUG_EMAIL2,'name'=>'Looyalty Emails"'));
            }
        }
        else
        {
    ...
    final public function getSigningDomain()
    {
    	return $this->signingDomain;
    }
    
    final public function setSigningDomain($_domain)
    { 
    	$this->signingDomain  = $_domain;
    }
